...
const Onboarding: React.FC<OnboardingProps> = ({ onProfileCreate, defaultName }) => {
  ...
  const OptionButton = ({ selected, onClick, label, sublabel }: any) => (
    <button
      onClick={onClick}
      className={`w-full p-4 sm:p-5 rounded-2xl border transition-all text-left flex items-center justify-between group mb-2 sm:mb-3 ${
        selected 
    ...
    >
      <div className="flex-1 pr-3">
        <p className={`font-bold text-sm sm:text-base ${selected ? 'text-emerald-700' : 'text-slate-700'}`}>{label}</p>
        {sublabel && <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5 sm:mt-1 leading-tight">{sublabel}</p>}
      </div>
      <div className={`w-5 h-5 rounded-full border-2 transition-all flex-shrink-0 flex items-center justify-center ${
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans antialiased overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 pb-8">
        <div className="w-full max-w-lg h-full flex flex-col max-h-[85vh]">
          
          <div className="mb-4 text-center">
            <span className="text-[10px] sm:text-[11px] font-black text-slate-300 uppercase tracking-[0.2em]">
              Etapa {step} de {TOTAL_STEPS}
            </span>
          </div>

          <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col flex-1 overflow-hidden">
            
            {/* Área de Conteúdo Rolável */}
            <div className="flex-1 overflow-y-auto px-6 py-8 sm:px-12 sm:py-10 custom-scrollbar">
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                {step === 1 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Para começar, qual o <span className="text-emerald-500">seu nome</span>?</h2>
                    <input autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-base font-bold outline-none" placeholder="Ex: Maria Silva" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                  </div>
                )}

                {step === 2 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Qual a sua <span className="text-emerald-500">idade</span>?</h2>
                    <input type="number" autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-xl font-black outline-none" value={formData.age} onChange={e => setFormData({...formData, age: Number(e.target.value)})} />
                  </div>
                )}

                {step === 3 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Como você se <span className="text-emerald-500">identifica</span>?</h2>
                ...
                {step === 4 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Qual o seu <span className="text-emerald-500">peso atual</span>?</h2>
                    <div className="relative">
                      <input type="number" autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-xl font-black outline-none pr-16" value={formData.weight} onChange={e => setFormData({...formData, weight: Number(e.target.value)})} />
                ...
                {step === 5 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Qual a sua <span className="text-emerald-500">altura</span>?</h2>
                    <div className="relative">
                      <input type="number" autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-xl font-black outline-none pr-16" value={formData.height} onChange={e => setFormData({...formData, height: Number(e.target.value)})} />
                      <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 uppercase tracking-widest">CM</span>
                    </div>
                  </div>
                )}
                ...
                {step === 6 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Prefere qual sistema de <span className="text-emerald-500">medida</span>?</h2>
                    <div className="space-y-1">
                ...
                {step === 7 && (
                  <div className="space-y-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Qual seu nível de <span className="text-emerald-500">movimento</span> diário?</h2>
                    <div className="space-y-1">
                ...
                {step >= 8 && step < 25 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-1 pb-4">
                      ...
                      {step === 9 && (
                        <input autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-base font-bold outline-none" placeholder="Ex: Musculação, Yoga..." value={formData.sportType} onChange={e => setFormData({...formData, sportType: e.target.value})} />
                      )}
                      {step === 10 && (
                        <>
                      ...
                      {step === 11 && (
                        <div className="relative">
                          <input type="number" autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-xl font-black outline-none pr-16" value={formData.desiredWeight} onChange={e => setFormData({...formData, desiredWeight: Number(e.target.value)})} />
                          <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 uppercase tracking-widest">KG</span>
                        </div>
                      )}
                      {step === 12 && (
                      ...
                      {step === 15 && (
                        <input autoFocus className="w-full px-5 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:bg-white focus:border-emerald-500 transition-all text-base font-bold outline-none" placeholder="Ex: Glúten, Lactose..." value={formData.allergies?.join(', ')} onChange={e => setFormData({...formData, allergies: e.target.value.split(',').map(s => s.trim())})} />
                      )}
                      {step === 16 && (
                        <>
                ...
                {step === 25 && (
                  <div className="text-center py-6 space-y-6">
                    <div className="w-20 h-20 bg-emerald-50 rounded-[2rem] flex items-center justify-center text-emerald-500 mx-auto shadow-sm">
                      <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    </div>
                    <h2 className="text-2xl font-black text-slate-900 leading-tight">Sua estratégia está <span className="text-emerald-500">pronta</span>!</h2>
                    <p className="text-slate-500 text-sm font-medium max-w-xs mx-auto leading-relaxed">Analisamos seu perfil e criamos um plano nutricional exclusivo baseado em ciência.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="px-6 py-6 sm:px-12 sm:py-8 border-t border-slate-50 bg-slate-50/30 backdrop-blur-sm">
              <div className="space-y-3">
                <button 
                  onClick={next} 
                  disabled={step === 1 && !formData.name}
                  className="w-full py-4 sm:py-5 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-30 disabled:cursor-not-allowed text-white rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest shadow-xl shadow-emerald-500/20 transition-all transform active:scale-[0.98]"
                >
                  {step === TOTAL_STEPS ? 'Começar Jornada' : 'Continuar'}
                </button>
                {step > 1 && (
                  <button 
                    onClick={prev} 
